...
class SpatialGrid{
  ...
  query(map,x,y,r){
    const out=[],c=this.cell,minX=Math.floor((x-r)/c),maxX=Math.floor((x+r)/c),minY=Math.floor((y-r)/c),maxY=Math.floor((y+r)/c);
    for(let gx=minX;gx<=maxX;gx++)for(let gy=minY;gy<=maxY;gy++){
      const a=map.get(gx+','+gy);if(a)for(let i=0;i<a.length;i++)out.push(a[i]);
    }
    return out;
  }
  rebuild(){
    ...
    for(const s of G.snakes){
      if(s.dead)continue;
      this.add(this.heads,s.x,s.y,s);
      for(let i=6;i<s.body.length;i+=3){const p=s.body[i];this.add(this.body,p.x,p.y,{s,p,i})}
    }
  }
