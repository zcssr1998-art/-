...
const {Game,Snake,Food}=require('./engine');
const {Renderer}=require('./render');
const {Input}=require('./input');
const {SIM}=require('./config');
const {installAdvanced}=require('./advanced');
const {installGiant}=require('./giant');
const {installVersionInfo}=require('./version');
installAdvanced({Game,Snake,Food,Renderer,Input});
installGiant({Game,Snake,Food,Renderer,Input});
installVersionInfo({Renderer,Input});
function boot(){
  ...
  const offResize=platform.onResize?platform.onResize(()=>{renderer.resize();input.layout();renderer.draw()}):()=>{};
  ...
  renderer.resize();renderer.draw();frameId=platform.raf(frame);const runtime={platform,game,renderer,input,stop:()=>{offResize();platform.caf(frameId)}};if(typeof GameGlobal!=='undefined')GameGlobal.EvoSnake=runtime;return runtime
}
module.exports={boot};